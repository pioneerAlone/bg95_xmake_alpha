...
/**
 *  HTTP extended config options supported by qapi_Net_HTTPc_Extended_Config_Options().
 */
typedef enum 
{
   QAPI_NET_HTTPC_EXTENDED_CONFIG_NONE = 0,
   /**< Extended config option -- not valid */
   QAPI_NET_HTTPC_EXTENDED_CONFIG_DNS_RESOLVE_INTERFACE,
   /**< Extended config option -- DSN resolve hostname on a specific interface */
